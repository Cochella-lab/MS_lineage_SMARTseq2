...
# Script purpose: functions for scATAC-seq clustering and annotation
# Usage example: 
...
##########################################
# download marker genes from wormbase using wormMine together with anatomical information 
##########################################
...
########################################################
########################################################
# Section : test methods of cell type assignment with prepred marker genes table
# 
########################################################
########################################################
run_cellassign = function(test = FALSE)
{
  if(test == TRUE){
    # test the installation using conda env
    ##########################################
    # IMPORTANT NOTE: 
    # It seems important that you set RETICULATE_PYTHON before you first use reticulate.
    ##########################################
    #Sys.setenv(RETICULATE_PYTHON = "/Users/jiwang/anaconda3/envs/cellassign_test/bin/python")
    library(reticulate)
    
    #py_discover_config() # check python verions found by reticulate
    #reticulate:::conda_list() # check conda env list
    #use_python("/Users/jiwang/anaconda3/envs/keras_tf/bin/python")
    #use_condaenv(condaenv = 'cellassign_test', conda = "/Users/jiwang/anaconda3/condabin/conda", required = TRUE)
    py_config()
    
    install.packages("tensorflow")
    library(tensorflow)
    tensorflow::install_tensorflow(extra_packages='tensorflow-probability', version = "2.0.0")
